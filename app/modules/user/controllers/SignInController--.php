...
use app\modules\user\models\form\ActivationForm;
...
class SignInController extends \rabint\controllers\DefaultController
{
     ...
     * @return string
     */
    public function actionActivation($identity = null, $redirect = null)
    {
        if(!Module::getConfig('enableActivation')){
            Yii::$app->session->setFlash('danger',Yii::t('rabint','امکان فعال فعال سازی وجود ندارد.'));
...
//            die('=---');
    ...
    {
        if ($redirect == null) {
            $redirect = Yii::$app->request->referrer ?: Yii::$app->homeUrl;
        }
