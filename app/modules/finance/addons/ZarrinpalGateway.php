...
use vendor\app\modules\finance\addons\nusoap_client;
use Yii;
...
//        ];
        ...
        $this->config = config('SERVICE.finance.gateways.zarrinpal');
    ...
    //
    public function startPay($orderId, $amount, $callbackUrl,$api=true)
    {
...
//        )));
        ...
        if($api)
            return ["method"=>"GET","url"=>$PayPath,"data"=>[]];
//        redirect($PayPath);
//        header('location:' . );
//        die();
        ?>
...
//        \Yii::warning('pay_get:' . print_r($_GET, TRUE) . ' - pay_post:' . print_r($_POST, TRUE), 'payCheck');
        ...
        $transaction = \app\modules\finance\models\FinanceTransactions::findOne($orderId);
