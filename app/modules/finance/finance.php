...
use app\modules\financeaddons\WalletGateway;
    ...
    public static function dashboardMenu()
    {
        ...
        if (config('SERVICE.finance.showDashboardMenu', true)) {
            return [
                ...
                'icon' => '<i class="fa fa-credit-card"></i>',
                'options' => ['class' => 'treeview'],
                'hit' => \Yii::t('app', 'موجودی حساب شما: ') . number_format($cash) . Yii::t('app', 'تومان'),
                'items' => [
                    //                [
                    //                    'label' => Yii::t('rabint', 'صورتحساب  ها'),
...
//        ];
    ...
    public static function getLatestTransactionStatus($removeSession = TRUE)
    {
        ...
        if ($_SESSION['finance']['pay_status'] != \app\modules\finance\Config::TRANSACTION_COMPLETED) {
            return false;
        }
        /* ------------------------------------------------------ */
    ...
    {
        return [
            static::CURRENCY_IR_RIAL => ['factor' => 1, 'title' => \Yii::t('rabint', 'تومان')],
